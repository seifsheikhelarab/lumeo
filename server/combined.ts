...
io.on("connection", (socket) => {
  ...
  socket.on("sync-request", ({ roomId }) => {
    ...
    hostSocket.emit("sync-request", { roomId });
  });

  socket.on("sync-response", ({ roomId, currentTime, isPlaying }) => {
    const room = rooms.get(roomId);
    if (!room) return;
    room.currentTime = currentTime;
    room.isPlaying = isPlaying;
    socket.to(roomId).emit("sync-response", { currentTime, isPlaying });
  });
  ...
  socket.on("rejoin-room", ({ roomId, userName }) => {
    const room = rooms.get(roomId);
    if (!room) { socket.emit("error", { message: "Room not found (may have been deleted)" }); return; }
    ...
    socket.emit("room-joined", {
      roomId, isHost: participant.isHost, participant,
      participants: Array.from(room.participants.values()),
      contentId: room.contentId, contentType: room.contentType,
      season: room.season, episode: room.episode,
      isStarted: room.isStarted, server: room.server,
    ...
    console.log(`${userName} rejoining room ${roomId}`);
  });
...
httpServer.listen(PORT, () => console.log(`Lumeo server running on http://localhost:${PORT}`));