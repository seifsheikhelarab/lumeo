...
export function meta({ loaderData }: Route.MetaArgs) {
  const title = loaderData?.show?.name || "TV Show";
  const overview = loaderData?.show?.overview?.slice(0, 160) || "";
  return [
    { title: `${title} - Lumeo` },
    { name: "description", content: overview },
    { property: "og:title", content: `${title} - Lumeo` },
    { property: "og:description", content: overview },
    { property: "og:image", content: loaderData?.show?.poster_path ? `https://image.tmdb.org/t/p/w500${loaderData.show.poster_path}` : "" },
      ...
      <section>
        ...
        <div className="space-y-2">
          {show.seasons.filter(s => s.season_number > 0).map((season, idx) => {
            ...
            return (
            <div key={season.id} className="t-acc border border-zinc-800 rounded-lg overflow-hidden animate-fade-in-up" style={{ animationDelay: `${idx * 50}ms` }} data-open={String(isOpen)}>
              <button
                onClick={(e) => toggleSeason(e, season.season_number)}
                className="t-acc-head w-full flex items-center gap-3 p-3 bg-zinc-900 hover:bg-zinc-800 active:scale-[0.99] transition-all duration-200 text-left"
                data-cuelume-toggle
              >
                {season.poster_path && (
                  <img
